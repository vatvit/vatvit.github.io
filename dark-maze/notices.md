...
<!--
  GENERATED by scripts/notices.sh — do not edit by hand.
  Regenerate when a dependency changes; scripts/check-notices.sh fails CI when
  these hashes no longer match the lockfiles.

  sources:
    src-tauri/Cargo.lock                    d4cdacd5ab54a0b8cf836d529873f1e9ade8c8586d491b60b5c9ca68e4e3663a
    packaging/package-lock.json             1a62aa221256773eccc64499627e83bf84ebfbdfed51cfdabf84960be3366095
-->

# Third-party notices

Dark Maze is covered by its own `LICENSE` (all rights reserved). The components
below are **not** — each remains under its own licence, reproduced or named
here because the terms require the notice to travel with the software.

## The browser version contains none of this

The game itself has **no third-party runtime dependency**. `package.json` has no
`dependencies` at all: every pixel, sound and maze is generated by code written
in this repository, and the CP437 face is hand-encoded rather than a bundled
font file. The web build therefore ships nothing that needs a notice.

What follows applies to the **desktop and mobile builds**, where a wrapper is
compiled around that same web build.

## Desktop — Tauri

The desktop application statically links **430 Rust crates**. They are
listed in full below with the licence each is offered under. Where a crate
offers a choice, Dark Maze relies on a permissive option.

### The 5 MPL-2.0 crates, named separately
...
These are the only components here that are not purely permissive. MPL-2.0 is
**file-level copyleft**: it permits linking into a proprietary application, and
its obligation attaches to modified MPL-covered files. **Dark Maze modifies none
of them** — they arrive as unmodified dependencies of the webview stack — so the
obligation is to say what they are and where to get them, which is what this
section does. Their source is on crates.io at the versions given.
...
The web build is wrapped by Capacitor, whose runtime ships inside the APK and
the iOS bundle:

| Component | Version | Licence |
|---|---|---|
| @capacitor/core | 6.2.1 | MIT |
| @capacitor/android | 6.2.1 | MIT |
| @capacitor/ios | 6.2.1 | MIT |
| @capacitor/app | 6.0.3 | MIT |

`@capacitor/cli` and `@tauri-apps/cli` are build tools. They produce the
artifacts and no part of them is inside one, so they are deliberately absent.

### Android libraries compiled into the APK

Gradle pulls these in where npm never sees them, so they are easy to miss:

| Library | Version | Licence |
|---|---|---|
| androidx.appcompat:appcompat | 1.6.1 | Apache-2.0 |
| androidx.core:core | 1.12.0 | Apache-2.0 |
| androidx.activity:activity | 1.8.0 | Apache-2.0 |
| androidx.fragment:fragment | 1.6.2 | Apache-2.0 |
| androidx.coordinatorlayout:coordinatorlayout | 1.2.0 | Apache-2.0 |
| androidx.core:core-splashscreen | 1.0.1 | Apache-2.0 |
| androidx.webkit:webkit | 1.9.0 | Apache-2.0 |
| org.apache.cordova:framework | 10.1.1 | Apache-2.0 |
| org.jetbrains.kotlin (stdlib, via kotlin-bom) | — | Apache-2.0 |

JUnit, Espresso and Mockito appear in the Gradle files under
`testImplementation` and `androidTestImplementation`. They are not compiled
into a release APK and are not listed.
...
- **ISC**, **Zlib**, **0BSD**, **Unlicense**, **Unicode-3.0** — as published by
  their respective stewards.

Ask at vatvit.dev+darkmaze@gmail.com for a copy of any notice you cannot reach.
